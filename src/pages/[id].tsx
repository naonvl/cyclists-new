...
const Page = (props) => {
  ...
  const handleSubmit = async () => {
    ...
    imageTemp.src = image
    // document.getElementById('__next').append(imageTemp)
    ...
    const req = await fetch('/api/createorder', {
      method: 'POST',
      headers: {
    ...
    if (res.success) {
      ...
      window.open(
        `https://cyclists-com.myshopify.com/cart/42778299957447:${Number(
          order
        )}`
      )
    }
  ...
  useEffect(() => {
    ...
    // setUid(window.location.search.replace('?', '').split('&')[0].split('=')[1])
    loadSvg({
      mobile: mobile,
      texture: texture,
      canvas: canvasRef,
      ...
      <div className='flex flex-col px-4 mx-auto lg:px-16 lg:flex-row max-w-[1400px]'>
        ...
        <div className='lg:w-1/2'>
          ...
          <div className='flex flex-col w-full mb-2 mt-7 md:flex-row md:gap-4'>
            ...
            >
              {submitLoading ? 'Loading...' : 'add to checkout'}
            </button>
          </div>
        </div>
